...
A decentralized application (DApp) is a computer application that runs on a distributed computing system such as a blockchain. DApps on the Conflux network can leverage the network's high speed and low fees to provide a smooth user experience. They can range from games to financial applications and more.
...
### 21. **Hash**
A hash is a function that converts an input of letters and numbers into an encrypted output of a fixed length. In the context of blockchain, a hash function is used to secure data. Each block in a blockchain has a unique hash, and any change to the block's data will result in a different hash.
