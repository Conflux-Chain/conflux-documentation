...
![Generating Merkle Tree](../imgs/nft-tutorials/merkle-tree.png)
...
![Merkle Proof](../imgs/nft-tutorials/merkle-tree-tool.png)
...
![Merkle Proof](../imgs/nft-tutorials/merkle-tree-proof.png)

We use the `MerkleVerification` library to verify this proof:
...
### Step-by-Step Guide: Merkle Tree NFT Whitelist on Conflux CoreSpace using Hardhat
...
1. **Initialize a New Hardhat Project:**
   ...
   Choose to create an empty Hardhat project when prompted.

2. **Install Dependencies:**

   You will need `ethers` for interacting with Ethereum-like networks, `@nomiclabs/hardhat-ethers` for integrating Hardhat with Ethers, and `@confluxfans/ethers` for Conflux support.

   ```bash
   npm install --save ethers @nomiclabs/hardhat-ethers @confluxfans/ethers
   ```

3. **Configure Hardhat for Conflux:**
...
2. **Deploy to Conflux CoreSpace:**

Run your script with Hardhat:
