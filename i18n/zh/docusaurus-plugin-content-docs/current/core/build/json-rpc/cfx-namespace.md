...
当编码<strong x-id=“1”>数值</strong>（整数，数字）时：使用最紧凑的表示方式编码为十六进制，并加上`“0x”`前缀。 零应表示为`"0x0"`。 例如：

* `0x41`（十进制的 65）
* `0x400`（十进制的 1024）
* <strong x-id=“1”>错误样例</strong>：`0x`（应该至少有一位数字 - 零是`“0x0”`）
* <strong x-id=“1”>错误样例</strong>：`0x0400`（不允许有前导零）
* <strong x-id=“1”>错误样例：</strong>：`ff`（缺少`0x`前缀）
