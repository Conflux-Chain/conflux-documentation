...
An `action` object corresponds to an operation executed by the underlying EVM, which could be a call (`call`), contract creation (`create`), or an internal transfer (`internal transfer`). Currently, there are several types of actions:
...
| author  | 地址                                                     | ✅        | The authorizer address |
...
| ------------- | ------------------------------ | -------- | ---------------- |
| space         | [Space Enum](./enums.md#space) |          |                  |
| 地址            | 地址                             |          | Contract address |
| balance       | Uint256                        |          |                  |
| refundAddress | 地址                             |          | Refund address   |
...
| ------------------- | ----------------------------- | -------- | -------------------------------- |
| type                | [ActionType Enum](./enums.md) |          |                                  |
| blockHash           | 哈希                            |          | Hash of block                    |
| epochHash           | 哈希                            |          | Hash of epoch                    |
| epochNumber         | QUANTITY                      |          | Number of epoch                  |
| transactionHash     | 哈希                            |          | Hash of transaction              |
...
| toEpoch     | QUANTITY or TAG                        | ✅        | the epoch number, or the string `"latest_state"`, `"latest_confirmed"`, `"latest_checkpoint"` or `"earliest"`, see the [epoch number parameter](#the-default-epochnumber-parameter). 搜索将持续到(包括)这个纪元数。                              |
| fromAddress | Base32                                 | ✅        |                                                                                                                                                                                                                                    |
| toAddress   | Base32                                 | ✅        |                                                                                                                                                                                                                                    |
| blockHashes | Array of Hash                          | ✅        | default: `null` Array of up to 128 block hashes that the search will be applied to. 如果不是`null`，则会覆盖 from/to 纪元 字段。                                                                                                                 |
