...
1. `fork_same_height_merge.py` 会创建一个不稳定的 TreeGraph，大约有 95000 个区块。 在TreeGraph中，它有三个分支，并且在每个分支的固定高度都有星形分叉。 It corresponds to one worst case
   scenario for the consensus processing engine. 预期的速度是在2019年的MacBook Pro上大约70区块/秒，在m5a.xlarge上大约45区块/秒。
...
execution after the speed stabilizes. 对于每个版本，我们都会运行这些脚本以确保没有性能回退。
